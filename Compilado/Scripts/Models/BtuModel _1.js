...
var btuContext =
{
    ...
    RegistrarDatosEmpresa: function (RazonSocial, NombreComercial, Actividad, CodigoPostal, Estado, Ciudad, Colonia, Domicilio, Rfc,
        ...
        $.ajax({
            type: "POST",
            url: urlServer + "Btu/RegistrarDatosEmpresa",
            data: {
                ...
                if (resp.Error === false) {
                    for (var i = 0; i < resp.Resultado.length; i++) {
                        self.listDatosPanelEmpresa.push({
                            Id_Empresa: resp.Resultado[i].Id_Empresa, Status: resp.Resultado[i].Status, Administrador: resp.Resultado[0].Administrador
                        });
                    }
                    callBackResult({ ressult: 'tgp', message: resp.MensajeError });
                }
    ...
    ObtenerDatosVacante: function (Id_Candidato, Id_Vacante, callBackResult) {
        let self = this;
    ...
    ObtenerGridVacantesCandidatos: function (Id, Desde, Hasta, callBackResult) {
        let self = this;
        self.listVacantesCandidato.length = 0;
        $.ajax({
            type: "POST",
            url: urlServer + "Btu/ObtenerGridVacantesCandidatos",
            data: { Id, Desde, Hasta, },
            success: function (resp) {
                if (resp.Error === false) {
                    for (var i = 0; i < resp.Resultado.length; i++) {
                        self.listVacantesCandidato.push({
                            Nombre: resp.Resultado[i].Nombre, Total: resp.Resultado[i].Total, Salario: resp.Resultado[i].Salario, Id: resp.Resultado[i].Id, Logotipo: resp.Resultado[i].Logotipo,
                            Status: resp.Resultado[i].objVacCand.Status, Empresa: resp.Resultado[i].Razon_social, Id_Vac_Cand: resp.Resultado[i].objVacCand.Id
                        });
                    }
                    callBackResult({ ressult: 'tgp', message: resp.MensajeError });
                }
    ...
    AplicarVacante: function (Id_Vacante, Id_Interesado, Status, CorreoEmpresa, CorreoCandidato, Candidato, Vacante, IdVacCand, callBackResult) {             
        $.ajax({
            type: "POST",
            url: urlServer + "Btu/AplicarVacante",
    ...
    ObtenerGridEmpresasRegistradas: function (callBackResult) {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.Error === false) {
                    for (var i = 0; i < resp.Resultado.length; i++) {
                        IndicaVacantes = resp.Resultado[i].Total === '0' ? 'red' : 'green';
    ...
    ObtenerDatosSesion: function (callBackResult) {
        let self = this;
        self.listDatosSesion.length = 0;
        $.ajax({
            type: "POST",
            url: urlServer + "Btu/ObtenerDatosSesion",
            data: {},
            success: function (resp) {
                if (resp.Error === false) {
                    for (var i = 0; i < resp.Resultado.length; i++) {
                        self.listDatosSesion.push({
                            Email: resp.Resultado[i].Email
                        });
                    }
                    callBackResult({ ressult: 'tgp', message: resp.MensajeError });
                }
